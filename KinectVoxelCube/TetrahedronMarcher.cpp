...
const VertexPosNormal CUBE_VERTICES[] =
{
	//Front face
...
Mesh TetrahedronMarcher::getVoxelMesh(gk2::DeviceHelper &device) const
{
	float meshWidth = 2.0f;
	float meshHeight = 2.0f;
	float meshLength = 2.0f;
	vector<VertexPosNormal> vertices;
	vector<unsigned int> indices;
	for (int z = 0; z < _length; ++z) {
		for (int y = 0; y < _height; ++y) {
			for (int x = 0; x < _width; ++x) {
				if (_grid[getArrayCoordinate(Array3DCoord(x, y, z))]) {
					int firstIndex = vertices.size();
					for (int i = 0; i < 24; ++i) {
						VertexPosNormal v;
						v.Pos = XMFLOAT3(CUBE_VERTICES[i].Pos);
						v.Pos.x = (-0.5 + ((float)x / _width)) * meshWidth + v.Pos.x * meshWidth / _width;
						v.Pos.y = (0.5 - ((float)y / _height)) * meshHeight + v.Pos.y * meshHeight / _height;
						v.Pos.z = (-0.5 + ((float)z / _length)) * meshLength + v.Pos.z * meshLength / _length;
						v.Normal = XMFLOAT3(CUBE_VERTICES[i].Normal);
						vertices.push_back(v);
					}
					for (int i = 0; i < 36; ++i) {
						indices.push_back(CUBE_INDICES[i] + firstIndex);
					}
				}
			}
		}
	...
	return Mesh(device.CreateVertexBuffer(vertices), sizeof(VertexPosNormal),
		device.CreateIndexBuffer(indices), indices.size());
}
